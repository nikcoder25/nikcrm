...
export default function Overview({ clients, tasks, deliverables, payments, keywords }) {
  ...
  const attention = [
    ...dueDeliverables.map((d) => ({ id: `d-${d.id}`, kind: "Deliverable", title: d.title || typeLabel(d.type), client: nameOf(d.client_id), due: d.due_date, tag: deliverableStatusLabel(d.status) })),
    ...dueTasks.map((t) => ({ id: `t-${t.id}`, kind: "Task", title: t.title, client: nameOf(t.client_id), due: t.due, tag: typeLabel(t.type) })),
    ...
    <div>
      ...
      <Panel>
        <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "16px 20px", borderBottom: BD, fontFamily: disp, fontSize: 15, textTransform: "uppercase" }}>
          ...
          {attention.length > 0 && <span style={{ marginLeft: "auto", fontSize: 12, fontWeight: 800, background: "#f7dede", color: "#c0392b", border: BDt, borderRadius: 20, padding: "2px 11px" }}>{attention.length} overdue</span>}
        </div>
        {attention.length === 0 ? (
          <Empty>All caught up — nothing overdue. 🎉</Empty>
        ) : (
          attention.map((a) => (
            <div key={a.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "13px 20px", borderBottom: "2px solid #f0ece2", flexWrap: "wrap" }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 800, color: "#c0392b" }}>Due {a.due}</span>
            </div>
          ))
        )}
      </Panel>
