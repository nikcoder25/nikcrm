...
export default function ClientScope({ client, retainers = [], deliverables = [], onChanged }) {
  ...
  const add = () => { if (!addType) return; guard(async () => { await saveRetainer(client.id, addType, Number(addQty) || 0); setAddType(""); setAddQty(""); onChanged("client_retainers"); }, "Scope added"); };
  const setQty = (type, qty) => guard(async () => { await saveRetainer(client.id, type, qty); onChanged("client_retainers"); }, "Scope updated");
  const remove = (id) => guard(async () => { await deleteRetainer(id); onChanged("client_retainers"); }, "Scope removed");
